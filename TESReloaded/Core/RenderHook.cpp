...
void RenderHook::TrackRenderReflections(NiCamera* Camera, ShadowSceneNode* SceneNode) {
	...
	ShadowData->x = -1.0f; // Disables shadows rendering for water reflections (the geo is rendered with the same shader used in the normal scene!)
	(this->*RenderReflections)(Camera, SceneNode);
	ShadowData->x = TheSettingManager->SettingsShadows.Exteriors.Quality;
...
void RenderHook::TrackWaterCullingProcess(NiAVObject* Object) {
	...
	if (Object->GetWorldBoundRadius() >= 100.0f) {
		NiBound* Bound = Object->GetWorldBound();
		if (!HasWater || (HasWater && Bound->Center.z + Bound->Radius > TheShaderManager->ShaderConst.Water.waterSettings.x)) (this->*WaterCullingProcess)(Object);
	}
...
void RenderHook::TrackRender(BSRenderedTexture* RenderedTexture, int Arg2, int Arg3) {
	...
	TheRenderManager->SetSceneGraph();
	TheShaderManager->UpdateConstants();
	if (TheSettingManager->SettingsMain.Develop.TraceShaders && TheKeyboardManager->OnKeyPressed(TheSettingManager->SettingsMain.Develop.TraceShaders)) Logger::Log("START FRAME LOG");
	if (TheSettingManager->SettingsMain.Develop.LogShaders && TheKeyboardManager->OnKeyPressed(TheSettingManager->SettingsMain.Develop.LogShaders)) Logger::Log("START FRAME LOG");
	(this->*Render)(RenderedTexture, Arg2, Arg3);
...
void CreateRenderHook() {
	...
	*((int*)&RenderFirstPerson)				= 0x00698B30;
	...
	WriteRelJump(0x0049C3FD, 0x0049C40B);  // Avoid to manage the cells culling for reflections
	if (TheSettingManager->SettingsMain.Main.AnisotropicFilter >= 2) {
